...
static __always_inline int icmpV4parse(struct xdp_md *ctx, void *untrust_V4, void *amasks)
{
...
#else
    struct ethhdr *l2hdr = NULL;
#endif

    // __com001
    if (data + L2_HLEN > data_end)
    ...
    if (data + L2_HLEN + sizeof(*iph) > data_end)
        MXDP_V4DROP
...
static __always_inline int icmpV6parse(struct xdp_md *ctx, void *untrust_V6, void *amasks)
{
    ...
    if (data + L2_HLEN + sizeof(*ip6h) > data_end)
        MXDP_V6DROP
...
static __always_inline int send_icmp4(struct xdp_md *ctx, in4_addr *saddr, const __u8 icmp4_type, const __u8 icmp4_code)
{
    ...
    if ((L2_HLEN + sizeof(struct iphdr) + ICMPV4_MAX_SIZE) < ((data_end - data) + headroom))
        MXDP_V4DROP
    ...
    orig_iph = data + L2_HLEN + sizeof(*iph) + sizeof(*icmph);
...
#ifdef TRUNK_PORT
    memcpy(l2hdr->h_dest, data + headroom + offsetof(struct vlan_ethhdr, h_source), ETH_ALEN);
    memcpy(l2hdr->h_source, data + headroom + offsetof(struct vlan_ethhdr, h_dest), ETH_ALEN);
    l2hdr->h_vlan_encapsulated_proto = ntohs(ETH_P_IP);
#else
    memcpy(l2hdr->h_dest, data + headroom + offsetof(struct ethhdr, h_source), ETH_ALEN);
    memcpy(l2hdr->h_source, data + headroom + offsetof(struct ethhdr, h_dest), ETH_ALEN);
    l2hdr->h_proto = ntohs(ETH_P_IP);
#endif
    iph->version = orig_iph->version;
    iph->ihl = 5;
    iph->id = orig_iph->id;
    iph->tot_len = *((__be16 *)(data + L2_HLEN + headroom + offsetof(struct iphdr, tot_len)));
    ...
    void *n_off = data + L2_HLEN + sizeof(*iph);
...
static __always_inline int sendV4icmp(struct xdp_md *ctx, in4_addr *saddr, const __u8 icmp4_type, const __u8 icmp4_code, __u8 deltattl)
{
    ...
    if (bpf_xdp_adjust_tail(ctx, 0 - ((data_end - data) - (L2_HLEN + msgroom))))
        MXDP_V4DROP
...
static __always_inline int send_icmp6(struct xdp_md *ctx, struct in6_addr *saddr, const __u8 icmp6_type, const __u8 icmp6_code)
{
    ...
    // check for respect MTU size of outgoing packet
    if ((L2_HLEN + sizeof(struct ipv6hdr) + ICMPV6_MAX_SIZE) < ((data_end - data) + headroom))
        MXDP_V6DROP
    ...
    memcpy(l2hdr->h_dest, data + headroom + offsetof(struct vlan_ethhdr, h_source), ETH_ALEN);
    memcpy(l2hdr->h_source, data + headroom + offsetof(struct vlan_ethhdr, h_dest), ETH_ALEN);
    l2hdr->h_vlan_encapsulated_proto = ntohs(ETH_P_IPV6);
#else
    memcpy(l2hdr->h_dest, data + headroom + offsetof(struct ethhdr, h_source), ETH_ALEN);
    memcpy(l2hdr->h_source, data + headroom + offsetof(struct ethhdr, h_dest), ETH_ALEN);
    l2hdr->h_proto = ntohs(ETH_P_IPV6);
#endif
